# -*- coding: utf-8 -*-
"""Test_AOT_Paper_code.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1jHImU2XgaJsw2hyguWMT6YyMIp0N3LjV
"""

from google.colab import drive
drive.mount('/content/drive')

!tar -xzvf /content/drive/MyDrive/vlcs.tar.gz
...
n_epochs = 1 # parser.add_argument("--n_epochs", type=int, default=200, help="number of epochs of training")
...
#               % (epoch, n_epochs, i, len(dataloader), d_loss.item(), g_loss.item())
    ...
    if (epoch+1) % 1 == 0:
            ...
            with torch.no_grad():
                ...
                class AdaptiveCostModel:
                    def __init__(self, input_dim):
                        ...
                        # Define the parameters of the adaptive cost model
                        self.theta = np.random.rand(input_dim)

                    def forward(self, x):
                        # Compute the adaptive cost based on the parameters theta
                        return np.dot(x, self.theta)
                # Initialize an adaptive cost model
                adaptive_cost_model = AdaptiveCostModel(input_dim=4)  # Adjust the input_dim based on the dimensionality of hash codes
                _min_wass=sys.maxsize
                _min_wass1=sys.maxsize
                cors_label=0
                cors_label1=0
                array_size=(4,4)
                ...
                for k_l in range(2127):
                  for k in range(31):
                    centers_cpu = centers[k].cpu()
                    num_array2 = np.array(centers_cpu[:np.prod(array_size)]).reshape(array_size)
                    trainB_cpu = trainB[k_l].cpu()
                    num_array = np.array(trainB_cpu[:np.prod(array_size)]).reshape(array_size)
                    testB_cpu=testB[k_l].cpu()
                    num_array1 = np.array(testB_cpu[:np.prod(array_size)]).reshape(array_size)
                    cost_matrix = np.zeros((len(num_array), len(num_array2)))
                    cost_matrix1 = np.zeros((len(num_array1), len(num_array2)))

                    for i in range(len(num_array)):
                        for j in range(len(num_array2)):
                            # Compute the adaptive cost using the adaptive cost model
                            adaptive_cost = adaptive_cost_model.forward(np.abs(num_array[i] - num_array2[j]))
                            cost_matrix[i, j] = adaptive_cost
                    for i in range(len(num_array1)):
                        for j in range(len(num_array2)):
                            adaptive_cost = adaptive_cost_model.forward(np.abs(num_array1[i] - num_array2[j]))
                            cost_matrix1[i, j] = adaptive_cost

                    # Compute the optimal transport plan using the adaptive cost matrix
                    # Note: You may need to adjust parameters such as the regularization strength (gamma)
                    #       based on your specific use case
                    gamma = 0.1  # Regularization strength
                    transport_plan = ot.emd([], [], cost_matrix)
                    transport_plan1 = ot.emd([], [], cost_matrix1)

                    wasserstein_distance = np.sum(transport_plan * cost_matrix)
                    wasserstein_distance1 = np.sum(transport_plan1 * cost_matrix1)
                    if _min_wass > wasserstein_distance:
                      _min_wass=wasserstein_distance
                      cors_label=k
                    if _min_wass1 > wasserstein_distance1:
                      _min_wass1 = wasserstein_distance1
                      cors_label1=k
                  train_labels[k_l]=cors_label
                  test_labels[k_l]=cors_label1
            ...
            #print(centers)
            #print(centers.size())
            #print("TrainB hashing code is:",trainB[0])
            #print("size of tensor is:",trainB.size())
            #print("Lables of trainB :",train_labels)
            #print("size of test",testB.size())
            #print("size of test_lables",test_labels.size())
            #print("maximum value in train_lables",max(train_labels))
